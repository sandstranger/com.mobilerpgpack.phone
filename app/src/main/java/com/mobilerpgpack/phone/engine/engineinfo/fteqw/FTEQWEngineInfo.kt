...
class FTEQWEngineInfo : SDL3EngineInfo( EngineTypes.FTEQW){
    ...
    override fun buildCustomCommandLineArgs () : Collection<String>{
        return mutableListOf<String>().apply {
            ...
            if (pathToResources.isNotEmpty() && File(pathToResources).exists()){
            ...
            if (gameType == FTEQWGames.Quake2 &&
                ...
                this += quake2GameType.directoryName
            }
            else if (pathToBaseGameDirectory.isNotEmpty()) {
                File(pathToBaseGameDirectory).also {
                    if (it.exists()){
                        this += BASE_GAME_COMMAND
                        this += it.name
                    }
